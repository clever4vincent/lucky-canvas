...
export default class SlotMachine extends Lucky {
  ...
  public play(): void {
    ...
    // 新增延迟配置（示例为200ms间隔）
    const delayStep = _defaultConfig.delayStep; // 可配置化
    this.startDelays = this.slots.map((_, index) => index * delayStep);
    this.startTimes = new Array(this.slots.length).fill(0);
  ...
  private run(num: number = 0): void {
    const { rAF, step, prizeFlag, _defaultConfig, cellAndSpacing, slots } =
      this;
    ...
    if (this.step === 0 && prizeFlag?.length === slots.length) {
      ...
      this.endCallback?.(
        this.prizes.find((prize, index) => index === flag) || void 0
      );
      return;
    }
    // 如果长度为 0 就直接停止游戏
    ...
    slots.forEach((slot, slotIndex) => {
      ...
      // 新增延迟判断
      if (currentRuntime < this.startDelays[slotIndex]) {
        this.startTimes[slotIndex] = 0; // 重置启动时间
        return; // 跳过该插槽
      }
      // 记录实际启动时间
      if (!this.startTimes[slotIndex]) {
        this.startTimes[slotIndex] = Date.now();
      }

      // 修改时间间隔计算
      const startInterval = Date.now() - this.startTimes[slotIndex];
