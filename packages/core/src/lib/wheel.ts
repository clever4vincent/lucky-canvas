...
export default class LuckyWheel extends Lucky {
  ...
  private selectCallback?: SelectCallbackType;
  ...
  private activeIndices: Set<number> = new Set(); // 改用集合存储多选索引
  /**
   * 游戏当前的阶段
   * step = 0 时, 游戏尚未开始
   * step = 1 时, 此时处于加速阶段
  ...
  private initComputed() {
    ...
    this.$computed(this, "_defaultStyle", () => {
      const style = {
        ...
        wordWrap: true,
        lengthLimit: "90%",
        activeBackground: "#FFF9C4", // 默认激活背景
   ...
   * @param e 事件参数
   */
  // protected handleClick1(e: MouseEvent): void {
  //   const { ctx } = this;
  //   ctx.beginPath();
  ...
  //     if (this.step === 0) this.startCallback?.(e);
  //     return;
  //   }

  //   // 检测扇形区域点击
  //   const angle = Math.atan2(y, x) + Math.PI / 2; // 转换为数学角度
  //   let currentAngle =
  //     (angle - getAngle(this.rotateDeg) + 2 * Math.PI) % (2 * Math.PI);

  //   let accumulated = 0;
  //   this.activeIndex = this.prizes.findIndex((prize) => {
  //     const prizeAng = getAngle(prize.angle!);
  //     if (
  //       currentAngle >= accumulated &&
  //       currentAngle < accumulated + prizeAng
  //     ) {
  //       return true;
  //     }
  ...
  //     this.draw(); // 触发重绘
  //     // 可以在此处触发自定义回调
  //     console.log("Selected sector:", this.activeIndex);
  //   }
  // }
  // 新增辅助函数 → 确保角度在0-360度之间
  ...
  protected handleClick(e: MouseEvent): void {
    ...
    const angle =
      (Math.atan2(y, x) + Math.PI / 2 + 2 * Math.PI) % (2 * Math.PI);

    const actualRotate = this.normalizeAngle(this.rotateDeg); // 新增取模

    const currentAngle =
      (angle - getAngle(actualRotate) + 2 * Math.PI) % (2 * Math.PI);
    ...
    if (foundIndex !== -1) {
      // 多选逻辑：存在则删除，不存在则添加
      if (this.activeIndices.has(foundIndex)) {
        this.activeIndices.delete(foundIndex);
      } else {
        this.activeIndices.add(foundIndex);
      }
      this.draw();
      // console.log("当前选中:", Array.from(this.activeIndices));
      this.selectCallback?.(Array.from(this.activeIndices));
    }
  }
  ...
  protected draw(): void {
    ...
    this.prizes.forEach((prize, prizeIndex) => {
      ...
      // 添加激活状态边框
      if (isActive) {
        ctx.strokeStyle = prize.activeBorder || this._defaultStyle.activeBorder;
      ...
      // 计算临时坐标并旋转文字
      let x = Math.cos(middleDeg) * this.prizeRadius;
      let y = Math.sin(middleDeg) * this.prizeRadius;
      ctx.translate(x, y);
      ctx.rotate(middleDeg + getAngle(90));
      // 绘制图片
      prize.imgs &&
  ...
  // 在LuckyWheel类中添加补偿系数
  private getPhaseCompensation(): number {
    return (
      this.prizes.reduce((sum, prize, index) => {
        const prizeAng = getAngle(prize.angle!);
        return index < this.prizeFlag! ? sum + prizeAng : sum;
      }, 0) - getAngle(90)
    );
  }
  /**
  ...
  private carveOnGunwaleOfAMovingBoat(): void {
    const { _defaultConfig, prizeFlag, prizeDeg, rotateDeg, prizes } = this;
    // 新增校验逻辑
    if (
      prizeFlag === undefined ||
      prizeFlag < 0 ||
      prizeFlag >= prizes.length
    ) {
    ...
    // 动态获取当前奖品角度（核心修改点）
    const currentPrizeDeg = prizes[prizeFlag].angle!;
    const currentPrizeAng = getAngle(currentPrizeDeg);

    // 计算累积角度（新增逻辑）
    let accumulatedDeg = 0;

    for (let i = 0; i < prizeFlag; i++) {
      accumulatedDeg += prizes[i].angle!;
    }
    // 调整停止范围计算（基于当前奖品角度）
    const stopRange =
      (Math.random() * currentPrizeDeg - currentPrizeDeg / 2) *
    ...
    while (++i) {
      ...
      const endDeg =
        360 * i -
        accumulatedDeg -
        currentPrizeDeg / 2 - // 当前奖品半角偏移
        rotateDeg - // 当前旋转角度
        _defaultConfig.offsetDegree - // 全局角度偏移
        stopRange;
      // const endDeg =
      //   360 * i -
      //   prizeFlag! * prizeDeg -
